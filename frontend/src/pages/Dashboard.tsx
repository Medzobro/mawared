  ...
  return (
    <div dir={isAr ? 'rtl' : 'ltr'} className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-[var(--text-primary)]"><span className="text-gradient">{t('dashboard.title')}</span></h1><p className="text-sm text-[var(--text-secondary)] mt-1">{t('dashboard.sales_analysis')}</p></div>
        <div className="hidden sm:flex items-center gap-2 text-xs text-[var(--text-muted)] bg-[var(--bg-elevated)] px-3 py-1.5 rounded-lg border border-[var(--border-subtle)]">
          ...
          <span>{t('dashboard.last_update')}: {fmtDate(new Date().toISOString(), locale)}</span>
        </div>
      </motion.div>
      {/* KPIs */}
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div variants={itemVariants}><KPICard title={t('dashboard.total_products')} value={kpis.active_products} change={0} icon={Package} iconColor="text-teal-600" iconBgColor="bg-teal-50 dark:bg-teal-950" index={0} lang={lang} /></motion.div>
        <motion.div variants={itemVariants}><KPICard title={t('dashboard.stock_value')} value={kpis.total_inventory_value} change={0} icon={DollarSign} iconColor="text-emerald-600" iconBgColor="bg-emerald-50 dark:bg-emerald-950" suffix={` ${curr}`} index={1} lang={lang} /></motion.div>
        <motion.div variants={itemVariants}><KPICard title={t('dashboard.daily_transactions')} value={kpis.today_transactions} change={0} icon={Receipt} iconColor="text-blue-600" iconBgColor="bg-blue-50 dark:bg-blue-950" index={2} lang={lang} /></motion.div>
        <motion.div variants={itemVariants}><KPICard title={t('dashboard.alerts')} value={kpis.low_stock_items + kpis.expiring_soon} change={0} icon={AlertTriangle} iconColor="text-amber-600" iconBgColor="bg-amber-50 dark:bg-amber-950" index={3} lang={lang} /></motion.div>
      </motion.div>
      {/* Bento Grid */}
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <motion.div variants={itemVariants} className="lg:col-span-2 card-glass card-glow rounded-2xl p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-teal-50 dark:bg-teal-950 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-teal-600" /></div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {transactions.slice(0, 7).map((trx: any, i: number) => {
              ...
              return (
                <motion.div key={trx.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 + 0.3 }} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-subtle)] hover:border-[var(--border-medium)] transition-colors">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-[var(--text-primary)] truncate">{trx.id}</p><p className="text-xs text-[var(--text-muted)]">{trx.customer || t('dashboard.customer')} · {isAr ? toArabicNumerals(trx.item_count) : trx.item_count} {t('dashboard.piece')}</p></div>
                  <div className="text-left"><p className={`text-sm font-semibold ${trx.type === 'purchase' || trx.type === 'adjustment' ? 'text-red-500' : 'text-emerald-500'}`}>{trx.type === 'purchase' || trx.type === 'adjustment' ? '-' : '+'}{fmtNum(trx.amount)} {curr}</p><p className="text-[10px] text-[var(--text-muted)]">{fmtDate(trx.date, locale)}</p></div>
                </motion.div>
              );
            })}
          </div>
        ...
        <motion.div variants={itemVariants} className="card-glass card-glow rounded-2xl p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="badge badge-danger text-[10px]">{isAr ? toArabicNumerals(alerts.filter((a: any) => !a.read).length) : alerts.filter((a: any) => !a.read).length} {t('dashboard.unread_alerts')}</span>
          </div>
          <div className="space-y-2 max-h-80 overflow-y-auto no-scrollbar">
            {alerts.map((alert: any, i: number) => (
              <motion.div key={alert.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 + 0.3 }} className={`flex items-start gap-3 p-3 rounded-xl border ${getAlertColor(alert.severity)} bg-opacity-30`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] opacity-60 mt-1">{fmtDate(alert.date, locale)}</p>
                </div>
              </motion.div>
            ))}
          </div>
